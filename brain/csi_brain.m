...
%% ORIGINAL MESH DEFINEMENT
%mesh matrices 
[p_inv,e_inv,t_inv] = initmesh(d1_inv,'hmax',l/25); 
%hmax property sets no element side greater than l/12 for a satisfactory granularity
%pdeplot(p,e,t);axis tight;axis equal;
...
for rep=1:2 %number of refinements
    ...
    it_inv = [find(t_inv(4,:)==3) find(t_inv(4,:)==4) find(t_inv(4,:)==5) find(t_inv(4,:)==6) find(t_inv(4,:)==7) find(t_inv(4,:)==8) find(t_inv(4,:)==9) find(t_inv(4,:)==10) find(t_inv(4,:)==11) find(t_inv(4,:)==13)];% find(t_inv(4,:)==12)];
...
%     U_mat4 = mat2cell(repmat(Td,1,Nsources),Nnodes_d,Nnodes_d*ones(1,Nsources)); %it will be taken out for the loop as it needs to be calculated only once and is the same
    ...
    check_re = find(real(er_back.*x_cur+er_back)<1);
    if(isempty(check_re)==0)
        x_cur_re(check_re) = (1-real(er_back(check_re))+imag(x_cur(check_re)).*imag(er_back(check_re)))./real(er_back(check_re));
    end
    
    check_re = find(real(er_back.*x_cur+er_back)>80);
    if(isempty(check_re)==0)
        x_cur_re(check_re) = (80-real(er_back(check_re))+imag(x_cur(check_re)).*imag(er_back(check_re)))./real(er_back(check_re));
    end
    
    check_im = find(imag(er_back.*x_cur+er_back)>0);
    if(isempty(check_im)==0)
        x_cur_im(check_im) = -(real(x_cur(check_im)).*imag(er_back(check_im))+imag(er_back(check_im)))./real(er_back(check_im));
    end
    
    check_im = find(imag(er_back.*x_cur+er_back)<-20);
    if(isempty(check_im)==0)
        x_cur_im(check_im) = -(20+real(x_cur(check_im)).*imag(er_back(check_im))+imag(er_back(check_im)))./real(er_back(check_im));
    end
...
contrast = er_back.*x_cur+er_back;
