...
gd=[3 4 -dis-d-0.5*l      -dis-0.5*l        -dis-0.5*l        -dis-d-0.5*l    -dis-0.5*l-d   -dis-0.5*l-d     dis+0.5*l+d    dis+0.5*l+d; %left vertical PML (R1)
    ...
    1 0  -(0.22/1.7)*param   (0.2/1.7)*param               0                0                  0                      0              0                0;     %center circle for umlaut profile with small radius (C1)
    1 0  -(0.22/1.7)*param   2*(0.2/1.7)*param             0                0                  0                      0              0                0;     %center circle for umlaut profile with large radius (C2)
    3 4 -2*(0.2/1.7)*param -(0.2/1.7)*param    -(0.2/1.7)*param   -2*(0.2/1.7)*param   -(0.22/1.7)*param   -(0.22/1.7)*param   (0.28/1.7)*param   (0.28/1.7)*param; %left rectangular part of umlaut (R6)
    3 4  (0.2/1.7)*param     2*(0.2/1.7)*param  2*(0.2/1.7)*param   (0.2/1.7)*param    -(0.22/1.7)*param   -(0.22/1.7)*param   (0.28/1.7)*param   (0.28/1.7)*param; %right rectangular part of umlaut (R7)
    ...
    1   -(0.2/1.7)*param    (0.53/1.7)*param     (0.1/1.7)*param       0    0    0    0    0    0; %upper left circle from umlaut (C3)
    1    (0.2/1.7)*param    (0.53/1.7)*param     (0.1/1.7)*param       0    0    0    0    0    0; %upper right circle from umlaut (C3)
    ]'; 
...
%% ORIGINAL MESH DEFINEMENT
%mesh matrices 
[p_inv,e_inv,t_inv] = initmesh(d1_inv,'hmax',l/22); 
%hmax property sets no element side greater than l/12 for a satisfactory granularity
%pdeplot(p,e,t);axis tight;axis equal;
...
radius = 0.9*l;%0.5*l; %radius of circle in which receivers are uniformly distributed
...
MD = sparse(ii,jj,ones(1,Nnodes_d),Nnodes_d,Nunknowns_inv);
...
for in=1:Nnodes_inv
    if(p_inv(1,in)>=gd_inv(4,1)&&p_inv(1,in)<=gd_inv(3,2)&&p_inv(2,in)>=gd_inv(9,3)&&p_inv(2,in)<=gd_inv(7,4)) %Incriment has meaning only in non PML areas
        Ez_inc(in,1:Nsources) = E0.*exp(-1i*k0.*(p_inv(1,in).*coord(1,1:Nsources).*sind(theta)+p_inv(2,in).*coord(2,1:Nsources).*cosd(theta))); 
    else
        Ez_inc(in) = 0;
    end
end
...
%     U_mat4 = mat2cell(repmat(Td,1,Nsources),Nnodes_d,Nnodes_d*ones(1,Nsources)); %it will be taken out for the loop as it needs to be calculated only once and is the same
    ...
    ita_dd = ita_dd^(-1);
    cost_fun(ir) = nom_1*ita_s+nom_2*ita_dd;
end
