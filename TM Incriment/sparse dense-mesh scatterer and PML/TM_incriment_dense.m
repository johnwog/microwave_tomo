...
R = zeros(Nunknowns,1);%charge matrix
S=zeros(3,3);
S_b=zeros(3,3);
T=zeros(3,3);
T_b=zeros(3,3);
            ...
            if(node_id(n(k))==1)%check if p node is an unknown
                if(node_id(n(m))==1)%check if q node is an unknown
                    ...
                    if(regions(ie)~=2&&regions(ie)~=3&&regions(ie)~=4)%check if located in a PML are if so there is no stimulation of incriment wave
                        R(index(n(k)),1) = R(index(n(k)),1)+(-S_b(k,m)+k0^2*T_b(k,m))*E0*exp(-1i*k0*p(1,n(k)));%p(1,n(m)) represent xcor of node that is on check
                    end
                else
                    ...
                    if(regions(ie)~=2&&regions(ie)~=3&&regions(ie)~=4)%check if located in a PML are if so there is stimulation of incriment wave
                        R(index(n(k)),1) = R(index(n(k)),1)-(S(k,m)-k0^2*T(k,m))*Ezs0(n(m))+(-S_b(k,m)+k0^2*T_b(k,m))*E0*exp(-1i*k0*p(1,n(k)));
                    else
                        R(index(n(k)),1) = R(index(n(k)),1)-(S(k,m)-k0^2*T(k,m))*Ezs0(n(m));
                    end
                end
            end
        end
